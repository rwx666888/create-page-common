...
* 支持按需自动注入、注册自定义组件；
* API文件支持按需增量更新，自动按需引入依赖函数，no never-used; （Tips:仅限成人框架）
...
* 支持自定义模板与模板数据（内置多种常用数据及格式）；支持配置默认模板；（Tips:内置模板中包含成人框架中自定义的模块）
...
* 支持自动识别匹配复合组件，例如双日历组件 data[Time]Range，同时支持手动配置，并高亮显示匹配关系；（Tips:仅限成人框架）
